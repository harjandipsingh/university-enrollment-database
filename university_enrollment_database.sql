...
CREATE TABLE Enrollment (
    ...
    CRN         CHAR(5),
    CONSTRAINT FK1_Enrollment FOREIGN KEY (StudentID)
        REFERENCES Students(StudentID),
    CONSTRAINT FK2_Enrollment FOREIGN KEY (DeptCode, CourseNum, Section)
        REFERENCES CourseSection(DeptCode, CourseNum, Section)
);
...
VALUES
    ...
    ('G1234567', 'Billy',   'Brown',   '9876 Cafe St.',           'Sacramento',  'CA', '20912', 'bbrown90@gmu.edu',  '555-555-5555', 'Graduate');
...
VALUES
    ...
    ('P006', 'Angelina', 'Andrews', 'D3579', 'Lavender Way',  'Manassas',    'VA', '21212', 'aandrews59@college.edu', '202-442-9872'),
...
/* QUERY 1: List the first and last name of all students. */
SELECT FirstName, LastName
FROM Students;


/* QUERY 2: Retrieve all the majors a specific student is pursuing (for example, StudentID = 1). */
SELECT M.MajorName
FROM StudentMajor SM
JOIN Major M ON SM.MajorID = M.MajorID
WHERE SM.StudentID = 'G1234567';

/* QUERY 3: Find the number of students in each major. */
SELECT M.MajorName, COUNT(SM.StudentID) AS [Number of Students]
FROM StudentMajor SM
LEFT JOIN Major M ON SM.MajorID = M.MajorID
GROUP BY M.MajorName;

/* QUERY 4: Display all students living in a specific state (for example, 'PA'). */
SELECT FirstName, LastName, State
FROM Students
WHERE State = 'VA';

/* QUERY 5: Find students who have not declared any major yet. */
SELECT S.StudentID, S.FirstName, S.LastName
FROM Students S
LEFT JOIN StudentMajor SM ON S.StudentID = SM.StudentID
WHERE SM.MajorID IS NULL;

/* QUERY 6: Calculate each student's GPA. */
SELECT
    S.FirstName,
    S.LastName,
    ROUND(CAST(AVG(E.CourseGrade) AS DECIMAL(3,2)), 2) AS [Student GPA]
FROM Enrollment E
